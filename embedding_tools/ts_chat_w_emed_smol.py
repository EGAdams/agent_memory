...
sys.path.append( f"{ HOME_DIR }/smolagents/" )
sys.path.append( f"{ HOME_DIR }/smolagents/scratchbook" )
from smol_tool_library import create_directory, delete_directory, delete_file, list_directory_contents, replace_code_block, read_file, write_file, get_current_directory
...
def smol_completion( task ):
    ...
    agent = CodeAgent(
        ...
        additional_authorized_imports=[ 'List', 'Any', 'Dict', 'logging', 'json', 'datetime', 'stat', 'time', 'random', 'unicodedata', 'collections', 'math', 're', 'itertools', 'statistics', 'queue' ], 
        model=model
    )
...
class CodeQueryProcessor:
    ...
    def process_user_request( self, user_request ):
        ...
        ai_text = smol_completion( prompt_text )
